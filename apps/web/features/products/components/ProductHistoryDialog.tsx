...
// Get product history from API
const getProductHistory = async (productId: string): Promise<ProductHistoryEntry[]> => {
  try {
    ...
    const response = await fetch(`http://localhost:3015/api/v1/products/${productId}/history`, { headers });
    if (response.ok) {
...
const getEventColor = (type: string, status?: string) => {
  switch (type) {
    case "production":
      return "primary";
    case "test":
      return status === "UNDER_TEST" ? "warning" : "success";
    case "shipping":
      return "info";
    case "service":
      return "secondary";
...
const ProductHistoryDialog = ({
  ...
  useEffect(() => {
    const loadHistory = async () => {
      if (product?.id) {
        try {
          const apiHistory = await getProductHistory(product.id);
        ...
        {tabValue === 0 && (
          <Box sx={{ position: "relative" }}>
            {history.map((entry, index) => (
              <Box
                key={entry.id}
                sx={{ display: "flex", position: "relative", mb: 3 }}
              >
            ...
            <Stack spacing={2}>
              ...
              <Paper sx={{ p: 2 }}>
                ...
                <List dense>
                  {history
                    .filter((h) =>
                      ["production", "test", "shipping"].includes(h.type)
                    )
                    .slice(0, 5)
                    .map((entry) => (
                      <ListItem key={entry.id}>
                        <ListItemIcon>{getEventIcon(entry.type)}</ListItemIcon>
                        <ListItemText
                          primary={entry.event}
                          secondary={new Date(
                            entry.timestamp
              ...
              <Paper sx={{ p: 2 }}>
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                  <Chip
                    label={
                      product?.currentStatus?.replace("_", " ") || "Unknown"
                    }
                ...
                >
                  ...
                  <Box>
                    ...
                    <Typography
                      variant="h4"
                      color={
                        historySummary.issues === 0
                          ? "success.main"
                          : historySummary.issues <= 2
                            ? "warning.main"
                            : "error.main"
                      }
                    >
