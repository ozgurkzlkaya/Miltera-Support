...
const generateMockHistory = (product: any): ProductHistoryEntry[] => {
  ...
  history.push({
    ...
    status: "ACTIVE",
    description:
      "Product successfully delivered and activated at customer site",
  ...
  if (product?.issues && product.issues.length > 0) {
    ...
    history.push({
      ...
      description:
        "Customer reported connectivity issues. Service request created",
  ...
  const now = new Date();
  const monthsSinceDelivery =
    (now.getTime() - deliveryDate.getTime()) / (1000 * 60 * 60 * 24 * 30);
    ...
    history.push({
      ...
      details: {
        ...
        nextMaintenance: new Date(
          maintenanceDate.getTime() + 6 * 30 * 24 * 60 * 60 * 1000
        ).toISOString(),
      },
    });
  }
