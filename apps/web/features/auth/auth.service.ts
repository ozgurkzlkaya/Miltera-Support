  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data && data.success && data.data && data.data.token && data.data.user) {
        ...
        // Router replace yerine location.assign/replace bazı tarayıcılarda state'i bozabiliyor.
        // Önce küçük bir bekleme ile AuthProvider'ın state güncellemesini tamamlayalım
        setTimeout(() => {
